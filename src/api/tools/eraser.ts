...
export class Eraser extends Tool {
  ...
  constructor(iApi: InstanceAPI) {
    ...
    this._eraserWidthProperty = new SliderProperty(
      'Size',
      1,
      10,
      toolState?.width ?? 1,
    ...
    this._eraserOpacityProperty = new SliderProperty(
      'Strength',
      0,
      100,
      toolState?.strength ?? 100,
      '%',
    )
  ...
  invokeAction(mouseEvent: GridMouseEvent, event: Events): void {
    ...
    if (grid && mouseEvent.isDragging && mouseEvent.isOnCanvas) {
      ...
      const coordsToDraw = grid.reflectCoordinates(
        { x, y },
        -(pxWidth - 1),
        -(pxWidth - 1),
      )
      ...
      coordsToDraw.forEach((c) => {
        this._drawGraphic
          .rect(c.x, c.y, pxWidth, pxWidth)
          .fill({ color: 0, alpha: this._eraserOpacityProperty.value / 100 })
      })
