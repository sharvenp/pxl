...
export class Shade extends Tool {
  ...
  constructor(iApi: InstanceAPI) {
    ...
    this._shadeTypeProperty = new RadioProperty(
      'Mode',
      [ShadeMode.LIGHTEN, ShadeMode.DARKEN],
      toolState?.mode ?? ShadeMode.LIGHTEN,
    )
    this._strengthProperty = new SliderProperty(
      'Strength',
      0,
      100,
      toolState?.strength ?? 10,
      '%',
    )
    this._brushWidthProperty = new SliderProperty(
      'Size',
      1,
      10,
      toolState?.width ?? 1,
  ...
  private _calculateColor(
    ...
    const r = Math.max(
      0,
      Math.min(255, color.r * (1 - strength) + lightOffset * strength),
    )
    const g = Math.max(
      0,
      Math.min(255, color.g * (1 - strength) + lightOffset * strength),
    )
    const b = Math.max(
      0,
      Math.min(255, color.b * (1 - strength) + lightOffset * strength),
    )
