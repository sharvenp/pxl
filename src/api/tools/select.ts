...
export class Select extends Tool {
  ...
  constructor(iApi: InstanceAPI) {
    ...
    this._resetButtonProperty = new ButtonProperty(
      'Reset',
      Events.SELECT_TOOL_RESET,
    )

    this._toolProperties = [this._resetButtonProperty]
  ...
  invokeAction(mouseEvent: GridMouseEvent, event: Events): void {
    ...
    if (grid && cursor) {
      ...
      if (!this._isSelected) {
        ...
        const w = Math.abs(
          x - Math.max(this._dragStartX, mouseEvent.coords.x) - 1,
        )
        const h = Math.abs(
          y - Math.max(this._dragStartY, mouseEvent.coords.y) - 1,
        )

        // draw preview rectangle
        if (this._isDragging) {
          cursor.cursorGraphic.rect(x, y, w, h)
          cursor.cursorGraphic.stroke({
            width: 1,
            color: CURSOR_PREVIEW_COLOR,
            alignment: 1,
          })
        }

        if (
          !mouseEvent.isDragging &&
          this._isDragging &&
          event === Events.MOUSE_DRAG_STOP
        ) {
          // dragging stopped, select area
          this._isSelected = true

          // get pixels within region
          const pixels: Array<SelectedRegionData> = grid
            .getPixelFrame({ x, y }, w, h)
        ...
        if (this._isPointInRegion(mouseEvent.coords)) {
          ...
          this._selectedRegion?.pixels.forEach((px) => {
            ...
            if (grid.contains(px.currentCoords)) {
              this._drawGraphic
                .rect(px.currentCoords.x, px.currentCoords.y, 1, 1)
                .fill(px.color)
            }
          })
          this._regionContainer!.addChild(this._drawGraphic)
  ...
  private _isPointInRegion(coords: PixelCoordinates): boolean {
    if (this._selectedRegion) {
      return (
        coords.x >= this._selectedRegion.x &&
        coords.x <= this._selectedRegion.x + this._selectedRegion.width &&
        coords.y >= this._selectedRegion.y &&
        coords.y <= this._selectedRegion.y + this._selectedRegion.height
      )
    } else {
      return false
    }
  ...
  private _updatePreview(): void {
    ...
    this._previewGraphic
      .rect(
        this._selectedRegion.x,
        this._selectedRegion.y,
