...
export class HistoryAPI extends APIScope {
  ...
  private readonly _historyStack: Record<
    string,
    Record<string, Array<ContainerChild>>
  >
  ...
  undo(): void {
    ...
    if (
      ...
      this._historyStack[frameId][layerId].length === MAX_HISTORY_SIZE
    ) {
      return
    }
    ...
    if (grid && !grid.empty) {
      ...
      if (
        topMostGraphic.label === PxlSpecialGraphicType.FROM_LOAD_STATE ||
        topMostGraphic.label === PxlSpecialGraphicType.FROM_CLONE
      ) {
        return
      }
  ...
  get canUndo(): boolean {
    ...
    return (
      grid.peek().label !== PxlSpecialGraphicType.FROM_LOAD_STATE &&
      grid.peek().label !== PxlSpecialGraphicType.FROM_CLONE
    )
  }
