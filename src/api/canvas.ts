...
export class CanvasAPI extends APIScope {
  ...
  constructor(iApi: InstanceAPI, pixi: Application) {
    ...
    const getCoords = (event: FederatedMouseEvent) => {
      const localPos = event.getLocalPosition(pixi.stage, {
        x: event.globalX,
        y: event.globalY,
      });
      return {
        x: Math.abs(Math.floor(localPos.x)),
        y: Math.abs(Math.floor(localPos.y)),
      };
    };
    ...
    pixi.stage
      .on("pointerdown", (event) => {
        ...
        if (event.button === 0) {
          isDragging = true;
          this.$iApi.event.emit(Events.MOUSE_DRAG_START, {
            coords,
            isDragging,
            isOnCanvas: true,
          });
      ...
      .on("pointerup", (event) => {
        ...
        if (event.button === 0) {
          isDragging = false;
          this.$iApi.event.emit(Events.MOUSE_DRAG_STOP, {
            coords,
            isDragging,
            isOnCanvas: true,
          });
      ...
      .on("pointermove", (event) => {
        ...
        if (!(coords.x === lastCell?.x && coords.y === lastCell?.y)) {
          this.$iApi.event.emit(Events.MOUSE_MOVE, {
            coords,
            isDragging,
            isOnCanvas: true,
          });
    ...
    if (this.$iApi.state.loadedState?.canvas) {
      this._mirrorX =
        this.$iApi.state.loadedState.canvas.settings.mirrorX ?? false;
      this._mirrorY =
        this.$iApi.state.loadedState.canvas.settings.mirrorY ?? false;
    }
  }
  ...
  destroy(): void {
    ...
    this._pixi.destroy(
      { removeView: true },
      { children: true, texture: true, context: true },
    );
  }
  ...
  private _handleResize() {
    if (this._pixi) {
      ...
      const m = new WebKitCSSMatrix(
        this._pixi.canvas.parentElement!.style.transform,
      );
