...
export class KeyBindAPI extends APIScope {
  ...
  private _processInputDown(evt: KeyboardEvent): void {
    ...
    let candidateKeyBinds = this._keyBinds.filter(
      (k) => k.onAction === KeyAction.DOWN,
    );
  ...
  private _processInputUp(evt: KeyboardEvent): void {
    ...
    const candidateKeyBinds = this._keyBinds.filter(
      (k) => k.onAction === KeyAction.UP && k.keys.includes(key),
    );
  ...
  private _initiailizeDefaultKeyBinds(): void {
    // Tools
    this.on("pencil-select", KeyAction.DOWN, [Key.DIGIT_1], () =>
      this.$iApi.tool.selectTool(ToolType.PENCIL),
    );
    this.on("eraser-select", KeyAction.DOWN, [Key.DIGIT_2], () =>
      this.$iApi.tool.selectTool(ToolType.ERASER),
    );
    this.on("picker-select", KeyAction.DOWN, [Key.DIGIT_3], () =>
      this.$iApi.tool.selectTool(ToolType.PICKER),
    );
    this.on("fill-select", KeyAction.DOWN, [Key.DIGIT_4], () =>
      this.$iApi.tool.selectTool(ToolType.FILL),
    );
    this.on("rectangle-select", KeyAction.DOWN, [Key.DIGIT_5], () =>
      this.$iApi.tool.selectTool(ToolType.RECTANGLE),
    );
    this.on("ellipse-select", KeyAction.DOWN, [Key.DIGIT_6], () =>
      this.$iApi.tool.selectTool(ToolType.ELLIPSE),
    );
    this.on("line-select", KeyAction.DOWN, [Key.DIGIT_7], () =>
      this.$iApi.tool.selectTool(ToolType.LINE),
    );
    this.on("shade-select", KeyAction.DOWN, [Key.DIGIT_8], () =>
      this.$iApi.tool.selectTool(ToolType.SHADE),
    );
    this.on("select-select", KeyAction.DOWN, [Key.DIGIT_9], () =>
      this.$iApi.tool.selectTool(ToolType.SELECT),
    );
    ...
    this.on("tool-alt-mode-down", KeyAction.DOWN, [Key.SHIFT], () =>
      this.$iApi.tool.toggleAltMode(true),
    );
    this.on("tool-alt-mode-up", KeyAction.UP, [Key.SHIFT], () =>
      this.$iApi.tool.toggleAltMode(false),
    );
