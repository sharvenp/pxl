...
export class GridAPI extends APIScope {
  ...
  constructor(iApi: InstanceAPI, pixi: Application) {
    ...
    if (canvasConfig.animator) {
      ...
      canvasConfig.animator.frames.forEach((frameConfig: any) => {
        const newFrame = new Container({
          eventMode: "none",
          label: frameConfig.label,
        });
        ...
        frameConfig.layers.forEach((layerConfig: any) => {
          ...
          const graphic = new Graphics({
            roundPixels: true,
            label: PxlSpecialGraphicType.FROM_LOAD_STATE,
          });
      ...
      this._activeLayer =
        this._drawLayers[this._activeFrame.label][this._activeIndex];

      // Load from state
      this._onionSkin = canvasConfig.animator.onionSkin ?? false;
      this._fps = canvasConfig.animator.fps ?? DEFAULT_FPS;
    } else {
      this._activeFrame = new Container({
        eventMode: "none",
        label: Utils.getRandomId(),
      });
    ...
    this._onionSkinContainer = new Container({
      eventMode: "none",
      label: "onionSkinContainer",
      visible: this._onionSkin,
    });
  ...
  destroy(): void {
    ...
    for (const key in this._drawLayers) {
      if (Array.isArray(this._drawLayers[key])) {
        this._drawLayers[key].forEach((layer) =>
  ...
  extractPixels(
    layerIdx: number | undefined = undefined,
    frameIdx: number | undefined = undefined,
  ): Uint8ClampedArray {
    const targetFrame =
      frameIdx === undefined ? this._activeFrame : this._frames[frameIdx];

    const pixelData = this._pixi.renderer.extract.pixels({
      target:
        layerIdx === undefined
          ? this._activeLayer
          : this._drawLayers[targetFrame.label][layerIdx],
      antialias: false,
  ...
  getPixelFrame(
    ...
    width: number,
    height: number,
  ): Array<[PixelCoordinates, RGBAColor]> {
    // Use extract to get the pixel data
    const pixelData = this._pixi.renderer.extract.pixels({
  ...
  getContainerPreview(container: Container): HTMLCanvasElement {
    return this._pixi.renderer.extract.canvas({
      ...
      frame: new Rectangle(
        0,
        0,
        this._pixi.canvas.width,
        this._pixi.canvas.height,
      ),
      resolution: 1,
  ...
  peek(): ContainerChild {
    const child = this._activeLayer.getChildAt<ContainerChild>(
      this._activeLayer.children.length - 1,
    );
    return child;
  }

  pop(): ContainerChild {
    const child = this._activeLayer.removeChildAt<ContainerChild>(
      this._activeLayer.children.length - 1,
    );
    return child;
  }
  ...
  floodFill(
    ...
    const validCoords = (c: PixelCoordinates) =>
    ...
    const getPixelRGBA = (c: PixelCoordinates): RGBAColor => {
      ...
      return {
        r:
          premultiplyFactor === 0
            ? 0
            : Math.round(pixelData.pixels[idx] / premultiplyFactor),
        g:
          premultiplyFactor === 0
            ? 0
            : Math.round(pixelData.pixels[idx + 1] / premultiplyFactor),
        b:
          premultiplyFactor === 0
            ? 0
            : Math.round(pixelData.pixels[idx + 2] / premultiplyFactor),
        a: pixelData.pixels[idx + 3],
      };
    };
  ...
  setActiveLayer(layerIdx: number): void {
    if (
      layerIdx < 0 ||
      layerIdx > this._drawLayers[this._activeFrame.label].length - 1
    ) {
      return;
    }
  ...
  addLayer(): void {
    ...
    while (
      this._drawLayers[this._activeFrame.label].some((l) => l.label === id)
    ) {
      // ensure unique id
  ...
  addFrame(): void {
    ...
    const newFrameLayer = new Container({
      eventMode: "none",
      label: Utils.getRandomId(),
    });
  ...
  removeFrame(frameIndex: number): void {
    ...
    this._drawLayers[frameToRemove.label].forEach((layer) =>
      layer.destroy({ children: true }),
    );
    ...
    if (frameToSet !== undefined) {
      this.setActiveFrame(
        this._frames.findIndex((f) => f.label === frameToSet.label),
      );
    }
  ...
  reorderFrames(frameOrder: Array<Container>): void {
    ...
    this._frameIndex = this._frames.findIndex(
      (f) => f.label === this._activeFrame.label,
    );
  ...
  cloneFrame(frameIndex: number): void {
    ...
    frameToDuplicate.children.forEach((_, i) => {
      ...
      const extract = Utils.gridToRectangles(
        this.extractPixels(i, frameIndex),
        this.width,
        this.height,
      );
      const graphic = new Graphics({
        roundPixels: true,
        label: PxlSpecialGraphicType.FROM_CLONE,
      });
