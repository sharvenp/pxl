<template>
  <div v-show="visible" class="absolute bottom-32 right-5 mb-16">
    <div
      class="p-5 w-1 h-4 rounded-lg border-4 border-gray transparent-swatch"
    ></div>
  </div>
  <div v-show="visible" class="absolute bottom-32 right-5 mb-16 z-10">
    <div
      ...
      @click="() => togglePicker()"
      class="p-5 w-1 h-4 rounded-lg border-4 border-gray-200 z-10"
      :style="{ backgroundColor: currentColor?.colorHex }"
    ></div>
  </div>
  <div
    v-show="showPicker"
    class="absolute border shadow-md m-5 bottom-60 right-0 z-20"
  >
    <ColorPicker
      default-format="hex"
    ...
    <div class="flex flex-row justify-end bg-white">
      <button
        class="mt-2 mb-2 me-2 py-1 px-3 rounded-lg border-2 hover:border-gray-300"
        @click="addColor"
      >
        Add
      </button>
  ...
  <div
    v-show="visible"
    class="absolute h-36 bottom-5 right-0 m-5 z-10 rounded border bg-white overflow-auto scrollbar scrollbar-thumb-stone-200 scrollbar-track-while scrollbar-thumb-rounded-full scrollbar-w-3"
  >
      ...
      <template v-for="item in palette" :key="item.colorHex">
        <div class="rounded-lg transparent-swatch">
          <button
            ...
            class="p-4 w-1 h-1 rounded-lg border-gray-100 border-4 hover:border-gray-300"
          ...
          <button
            v-else
            class="p-4 w-1 h-1 rounded-lg border-orange-200 border-4"
            :style="{ backgroundColor: item.colorHex }"
            @click.left="selectColor(item)"
            @click.right="removeColor(item)"
          ></button>
