<template>
  <div class="title-bar absolute w-full grid grid-rows-2 pb-1 z-20">
    <div class="mt-1 flex flex-row text-sm justify-between">
      ...
      <div
        v-show="ipcSupported"
        class="row-span-2 flex flex-row justify-end text-sm mx-1"
      >
        <button
          class="px-2 self-center bg-stone-300 hover:bg-stone-400"
          @click="windowAction(WindowActionType.MINIMIZE)"
        >
          _
        </button>
        <button
          class="px-2 self-center bg-stone-300 hover:bg-stone-400"
          @click="windowAction(WindowActionType.MAXIMIZE)"
        >
          O
        </button>
        <button
          class="px-2 self-center bg-stone-300 hover:bg-stone-400"
          @click="windowAction(WindowActionType.CLOSE)"
        >
          X
        </button>
      </div>
    ...
    <div class="mt-1 flex flex-row text-sm relative">
      <div
        v-for="(options, menu) in menuOptions"
        :key="menu"
        class="relative mx-1"
        @click="openMenu(menu)"
      >
          ...
          <ul>
            <li
              v-for="option in options.filter(
                (o) =>
                  !ipcRequired.has(o.key) ||
                  (ipcSupported && ipcRequired.has(o.key)),
              )"
              :key="option.key"
            ...
            >
              {{ option.label }}
              <span v-if="option.hotkey" class="text-xs underline text-gray-400"
                >({{ option.hotkey }})</span
              >
            </li>
          </ul>
...
const menuOptions = ref<Record<string, Array<MenuOption>>>({
  File: [
    {
      key: MenuOptionType.NEW_PROJECT,
      label: "New Project",
      hotkey: "Ctrl+N",
      disabled: () => false,
    },
    {
      key: MenuOptionType.OPEN_PROJECT,
      label: "Open Project",
      hotkey: "Ctrl+O",
      disabled: () => false,
    },
    {
      key: MenuOptionType.SAVE_PROJECT,
      label: "Save Project",
      hotkey: "Ctrl+S",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.SAVE_PROJECT_AS,
      label: "Save Project As",
      hotkey: "Ctrl+Shift+S",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.EXPORT,
      label: "Export",
      hotkey: "Ctrl+E",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.EXPORT_FRAMES,
      label: "Export Frames",
      hotkey: "Ctrl+Shift+E",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.EXIT,
      label: "Exit",
      hotkey: "",
      disabled: () => false,
    },
  ...
  Edit: [
    {
      key: MenuOptionType.UNDO,
      label: "Undo",
      hotkey: "Ctrl+Z",
      disabled: () =>
        iApi?.history !== undefined ? !iApi?.history.canUndo : true,
    },
    {
      key: MenuOptionType.REDO,
      label: "Redo",
      hotkey: "Ctrl+Y",
      disabled: () =>
        iApi?.history !== undefined ? !iApi?.history.canRedo : true,
    },
  ],
  View: [
    {
      key: MenuOptionType.ZOOM_IN,
      label: "Zoom In",
      hotkey: "Ctrl++",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.ZOOM_OUT,
      label: "Zoom Out",
      hotkey: "Ctrl+-",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.RESET_VIEW,
      label: "Reset View",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_TOOLS_PANEL,
      label: "Toggle Tools",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_PALETTE_PANEL,
      label: "Toggle Palette",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_LAYERS_PANEL,
      label: "Toggle Layers",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_PREVIEW_PANEL,
      label: "Toggle Preview",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_CANVAS_SETTINGS_PANEL,
      label: "Toggle Canvas Settings",
      hotkey: "",
      disabled: () => !iApi?.initalized,
    },
    {
      key: MenuOptionType.TOGGLE_ANIMATOR_PANEL,
...
function handleClick(evt: MouseEvent) {
  ...
  let menuOptions = document.getElementById(
    `title-bar-${activeMenu.value}-options`,
  );
  if (
    (menu && menu.contains(evt.target as Node)) ||
    (menuOptions && menuOptions.contains(evt.target as Node))
  ) {
    return;
  }
