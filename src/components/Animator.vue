      ...
      <div class="flex items-center gap-4 mb-4">
        ...
        >
          ...
          <input
            ...
            min="1"
            max="60"
            class="accent-stone-500 border w-12 ps-2"
            :value="fps"
            @input="updateFps"
          />
        </label>
        <span class="text-xs text-gray-500 ml-auto"
          >{{ frames.length }} / {{ MAX_FRAME_COUNT }}</span
        >
      </div>
    </div>
    <div>
      ...
      >
        <div
          ...
          :class="{
            'border-stone-500 border-2': selectedFrame === i,
            'border-stone-200': selectedFrame !== i,
          }"
        >
  ...
  >
    ...
    <div class="relative z-10 bg-white p-2">
      ...
      <div class="flex justify-between text-sm mt-2">
        <span>{{ fps }} FPS</span>
        <span> {{ previewFrameId + 1 }} / {{ frames.length }} </span>
      </div>
    </div>
  </div>
</template>
...
onMounted(() => {
  handlers.push(
    iApi?.event.on(Events.CANVAS_FRAME_SELECTED, () => {
  ...
  handlers.push(
    iApi?.event.on(Events.CANVAS_UPDATE, () => {
